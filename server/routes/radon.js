...
import { createRadonSet } from '../radonIntake.js';
...
r.post('/tests', requireAuth, wrap(async (req, res) => {
  ...
  // The set itself is built in radonIntake.js, so a set opened here and one
  // that arrives from a phone are the same set, made the same way.
  const out = await tx(async (c) => createRadonSet(c, b));
